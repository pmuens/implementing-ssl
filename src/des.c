...
// This does not return 1 for a 1 bit; it just retuns non-zero.
 ...
 * respectively, but src can be shorter (expansion function depends on
 * this).
 *
...
/**
 * Perform the left rotation operation on the key. This is made fairly complex
 * by the fact that the key is split into two 28-bit halves, each of which has
 * to be rotated independently (so the second rotation operation starts in the
 * middle of byte 3).
 */
...
static void ror(unsigned char *target) {
  ...
  target[3] =
      (((target[3] >> 1) | ((target[2] & 0x01) << 7)) & ~0x08) | carry_right;
...
static void des_block_operate(const unsigned char plaintext[DES_BLOCK_SIZE],
  ...
  for (round = 0; round < 16; round++) {
    ...
    // "Expansion". This permutation only looks at the first four bytes
    // (32 bits of ip_block); 16 of these are repeated in expansion_table.
    ...
    substitution_block[0] |= sbox[1][(expansion_block[0] & 0x03) << 4 |
                                     (expansion_block[1] & 0xF0) >> 4];
    substitution_block[1] = sbox[2][(expansion_block[1] & 0x0F) << 2 |
    ...
    substitution_block[2] |= sbox[5][(expansion_block[3] & 0x03) << 4 |
                                     (expansion_block[4] & 0xF0) >> 4];
    substitution_block[3] = sbox[6][(expansion_block[4] & 0x0F) << 2 |
...
void des_encrypt(const unsigned char *plaintext, const int plaintext_len,
  ...
  des_operate(plaintext, plaintext_len, ciphertext, (const unsigned char *)iv,
              key, OP_ENCRYPT, 0);
}
...
void des_decrypt(const unsigned char *ciphertext, const int ciphertext_len,
                 unsigned char *plaintext, void *iv, const unsigned char *key) {
  des_operate(ciphertext, ciphertext_len, plaintext, (const unsigned char *)iv,
              key, OP_DECRYPT, 0);
}

void des3_decrypt(const unsigned char *ciphertext, const int ciphertext_len,
...
/*
./dist/des -e password initialz abcdefgh
71828547387b18e5

./dist/des -d password initialz 0x71828547387b18e5
6162636465666768

./dist/des -e twentyfourcharacterinput initialz abcdefgh
c0c48bc47e87ce17

./dist/des -d twentyfourcharacterinput initialz 0xc0c48bc47e87ce17
6162636465666768
*/
