 ...
 * pointers to the host part and the path part. Note that this function modifies
 * the uri itself as well. It returns 0 on success, -1 if the URL is found to be
...
int parse_proxy_param(char *proxy_spec, char **proxy_host, int *proxy_port,
  ...
  // If the user added a "/" to the end, just ignore it.
  ...
  if (colon_sep) {
    ...
    *proxy_port = atoi(colon_sep + 1);
    if (*proxy_port == 0) {
      // 0 is not a valid port; this is an error.
      return 0;
    }
  } else {
...
int main(int argc, char *argv[]) {
  ...
  if (!strcmp("-p", argv[ind])) {
    if (!parse_proxy_param(argv[++ind], &proxy_host, &proxy_port, &proxy_user,
                           &proxy_password)) {
